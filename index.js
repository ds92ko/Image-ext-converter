...
fs.readdir(inputFolderPath, (err, files) => {
  ...
  imageFiles.forEach((imageFileName) => {
    ...
    const webpFileName =
      path.basename(imageFileName, path.extname(imageFileName)) + ".webp";
    const webpFilePath = path.join(outputFolderPath, webpFileName);

    sharp(imageFilePath).toFile(webpFilePath, (conversionErr, info) => {
      if (conversionErr) {
        console.error(
          `이미지 변환 오류 (${imageFileName}): ${conversionErr.message}`
        );
      } else {
        console.log(
          `이미지 변환 성공 (${imageFileName}). 출력 파일: ${webpFilePath}`
        );
